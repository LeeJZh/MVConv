...
import argparse
from rfnet.rfnet_builder import RFNetBuilder
from ndp_train import train_main
from rfnet.rfnet_fusion import convert_rfnet_weights
import os
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--block_type', default='rfb')
    parser.add_argument('-c', '--conti_or_fs', default='fs')
    parser.add_argument('-e', '--epochs', default=400, type=int)
    parser.add_argument('--alpha', default=1e-4, type=float)
    parser.add_argument('--scale', default=2, type=int)        # continue or train_from_scratch
    parser.add_argument(
    ...
    assert block_type in ['rfb', 'base']
    auto_continue = conti_or_fs == 'continue'
    print('auto continue: ', auto_continue)
    alpha = start_arg.alpha
    scale = start_arg.scale
    epochs = start_arg.epochs
    ...
    if network_type == 'sres18':
        ...
        #   94.47  --->  95+
        batch_size = 192
        lrs = LRSchedule(base_lr=0.3, max_epochs=epochs, lr_epoch_boundaries=None, lr_decay_factor=None,
                         linear_final_lr=None, cosine_minimum=0)
        warmup_epochs = 0
        gamma_init = 0.333
        #   --------------------------------------
