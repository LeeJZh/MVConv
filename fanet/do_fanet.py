...
import argparse
from fanet.fanet_builder import FANetBuilder
from ndp_train import train_main
from fanet.fanet_fusion import convert_fanet_weights
import os
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--block_type', default='fab')
    ...
    log_dir = 'fanet_exps/{}_{}_train'.format(network_type, block_type)
    ...
    if block_type == 'fab':
        builder = FANetBuilder(base_config=config, deploy=False, gamma_init=gamma_init)
    else:
    ...
    if block_type == 'fab':
        convert_fanet_weights(target_weights, target_weights.replace('.hdf5', '_deploy.hdf5'), eps=1e-5)
        deploy_builder = FANetBuilder(base_config=config, deploy=True)
