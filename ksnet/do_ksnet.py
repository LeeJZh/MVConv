...
import argparse
from ksnet.ksnet_builder import KSNetBuilder
from ndp_train import train_main
from ksnet.ksnet_fusion import convert_ksnet_weights
import os
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', '--block_type', default='ksb')
    ...
    log_dir = 'ksnet_exps/{}_{}_train'.format(network_type, block_type)
    ...
    if block_type == 'ksb':
        builder = KSNetBuilder(base_config=config, deploy=False, gamma_init=gamma_init)
    else:
    ...
    if block_type == 'ksb':
        convert_ksnet_weights(target_weights, target_weights.replace('.hdf5', '_deploy.hdf5'), eps=1e-5)
        deploy_builder = KSNetBuilder(base_config=config, deploy=True)
